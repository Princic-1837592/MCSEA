[10/10 solution](https://studio.code.org/projects/applab/ooKWulwfnApNNF-_uO20IlG8j6_N5zulXDfCZRvWXsM)
 ...
 * Parses an equation in the given form: "T1 + T2 + ... + Tn = Tn+1 + Tn+2 + ... + Tn+m"
 * and returns an array of the terms.
 * Each term is an array of elements of the form:
 * {
 *     element: string,
 *     number: integer
 * }
 * Group coefficients will be applied to each single element of the term.
 * Terms on the right will have teir coefficient multiplied by -1 to simplify the balancing process.
 *
 * For example the given equation "Na + 2ClH = HNaCl" would be parsed to:
 * [
 *   [{element: "Na", number: 1}],
 *   [{element: "Cl", number: 2}, {element: "H", number: 2}],
 *   [{element: "H", number: -1}, {element: "Na", number: -1}, {element: "Cl", number: -1}]
 * ]
 */
function parse_equation(equation_str) {
    var split = equation_str.split("=");
    var left = split[0];
    var right = split[1];
    var left_terms_str = left.split("+").map(function(term) {
        return term.trim()
    });
    var left_terms = left_terms_str.map(function(term) {
        return parse_term(term, 1)
    });
    var right_terms_str = right.split("+").map(function(term) {
        return term.trim()
    });
    var right_terms = right_terms_str.map(function(term) {
        return parse_term(term, -1)
    });
    return left_terms.concat(right_terms);
}

var term = /(?<coefficient>\d*)(?<element>[A-Z][a-z]?)(?<number>\d*)/g;

/**
 * GIVEN BY THE PROFESSOR
 * Given a string representing a composition o elements like 2H2O3,
 * returns a list of lists in which each list contains information about one element
 * and has the following structure:
 * [single element with coefficients, group coefficient, element symbol, element number]
 ...
 * ]
 *
 * ACTUALLY JUST A REPLACEMENT FOR matchAll THAT APPARENTLY IS NOT WORKING ON code.org
 */
function match_all(term_str) {
    var matches = [];
    var match;
    while (match = term.exec(term_str)) {
        matches.push(match);
    }
    return matches;
}
...
/**
 * GIVEN BY THE PROFESSOR
 * Prses a single term and returns an array of elements with the following structure:
 * {
 *     element: string,
 *     number: integer
 * }
 * Coefficient is 1 for the terms on the left and -1 for the terms on the right.
 */
function parse_term(term_str, coefficient) {
    var elements = [];
    var matches = match_all(term_str);
    // the coefficient applied to the whole term, like 2 in 2NaCl
    var group = undefined;
    for (var match in matches) {
        match = matches[match];
        // group must be the same for the whole term
        group = group || parseInt(match[1]) || 1;
        elements.push({
            element: match[2],
            number: coefficient * group * (parseInt(match[3]) || 1)
        })
    }
    return elements;
}
...
/**
 * Handles the input of the atomic number in the main screen.
 * Updates the displayed element to the correct one.
 * Default: Hydrogen
 */
onEvent("atomic_number", "input", function(event) {
    var element = data[getText("atomic_number")] || data["1"];
    setText("symbol", element.symbol);
    setText("name", element.name);
    setText("atomic_mass", element.atomic_mass);
    setText("shells", element.shells.join("\n"));
    setText("discovered_by", "Discovered by:\n" + element.discovered_by);
    showElement("summary");
    setText("summary", element.summary);
});

/**
 * Handles the input from the slider and updates the displayed number
 */
onEvent("limit", "input", function() {
    setText("limit_text", getNumber("limit"));
});

/**
 * Handles the pression of the button in the main screen
 */
onEvent("go_to_balance", "click", function(event) {
    setScreen("balance");
});

/**
 * Handles the pression of the button to go back to main screen from the balancing screen
 */
onEvent("go_to_main", "click", function(event) {
    setScreen("main");
});

/**
 * Checks if an equation is valid or not. Is not 100% accurate because doesn't check for something like:
 * H2b
 * which is not a valid element (b) but still filters out the majority of cases
 */
var equation_verifier = /\d*[A-Z][A-Za-z0-9]*( *\+ *\d*[A-Z][A-Za-z0-9]*)* *= *\d*[A-Z][A-Za-z0-9]*( *\+ *\d*[A-Z][A-Za-z0-9]*)*/;

onEvent("balance_btn", "click", function(event) {
    var input = getText("equation");
    if (!equation_verifier.exec(input)) {
        setText("solution", "Input is not a valid equation");
        return;
    }
    setText("solution", "Solving...");
    var solution = solve_equation(input, getNumber("limit"));
    if (solution)
        setText("solution", solution.join(", "));
    else
        setText("solution", "Cannot find solution");
});

/**
 * Counts the number of atoms for each element in the given equation.
 * Since terms in the right part of the equation have teir coefficient multiplied by -1,
 * if the equation is balanced all the elements will have the sum equal to 0.
 */
function count_elements(equation) {
    var elements = {};
    for (var term in equation) {
        for (var element in equation[term]) {
            element = equation[term][element];
            // the first time an element is found its number is set to 0
            if (!elements[element.element]) {
                elements[element.element] = element.number;
            } else {
                // the number of atoms for the element is incremented
                elements[element.element] += element.number;
            }
        }
    }
...
/**
 * Checks if the given equation is balanced by checking if all the elements have number 0 in the equation.
 */
function is_balanced(equation) {
    var elements = count_elements(equation);
...
/**
 * Returns the list of possible permutations of coefficients given the number of terms and the maximum value that can be used.
 * Coefficients are generated as numbers starting from 0 up to limit^terms (excluded).
 * Each number is then converted to a series of coefficients, applying the following algorithm:
 * 1. The number is divided by the limit.
 * 2. The remainder is added to the list of coefficients.
 * 3. The quotient is the next number on which the algorithm is applied.
 * This is repeated for terms times.
 */
function generate_coefficients(terms, limit) {
    var max = Math.pow(limit, terms);
    var result = [],
        coefficients = [];
    // coefficients are initialized as a 0-vector of length terms
    ...
    for (i = 0; i < max; i++) {
        n = i;
        // insert coefficients from right to left
        for (var j = terms - 1; j >= 0; j--) {
            // the +1 is to avoid having 0 as a coefficient and to include limit as a coefficient
            coefficients[j] = n % limit + 1;
            n = Math.floor(n / limit);
        }
        // the array of coefficients is copied
        result.push(coefficients.concat());
    }
    return result;
}

/**
 * Returns a new term after applying a coefficient to it.
 */
function apply_coefficient(coefficient, term) {
    var new_term = [];
    for (var element in term) {
        new_term.push({
            element: term[element].element,
            number: coefficient * term[element].number
        })
    }
    return new_term;
}
...
/**
 * Returns a new equation after applying a list coefficients to it.
 */
function apply_coefficients(coefficients, equation) {
    var new_equation = [];
    for (var i in coefficients) {
        new_equation.push(apply_coefficient(coefficients[i], equation[i]));
    }
    return new_equation;
}

/**
 * Uses brute force to solve the equation and returns the list of coefficients that produce the solution.
 */
function solve_equation(equation_str, limit) {
    var equation = parse_equation(equation_str);
    if (!equation) return;
    var terms = equation.length;
    var coefficients = generate_coefficients(terms, limit);
    for (var i in coefficients) {
        if (is_balanced(apply_coefficients(coefficients[i], equation))) {
 ...
 * ]
 * 
 * ATTENTION: this function does not check if the given string is a valid equation
 */
function parse_equation(equation_str) {
    var split = equation_str.split("=");
    var left = split[0];
...
onEvent("balance_btn", "click", function (event) {
    setText("solution", "Solving...");
    var s = solve(getText("equation"));
    if (s)
        setText("solution", s.join(", "));
    else
        setText("solution", "Cannot find solution");
});
...
function count(e) {
    ...
    for (var t in e) {
        for (var el in e[t]) {
            ...
            r[el.element] += el.number;
        }
    }
    return r;
}

function is_balanced(e) {
    var c = count(e);
    for (var el in c) {
        if (c[el] !== 0) {
            return false;
...
function coefficients(c, e) {
    var r = [];
    for (var i in c) {
        r.push(coefficient(c[i], e[i]));
    }
    return r;
}

function solve(e) {
    var eq = parse_equation(e);
    var t = eq.length;
    if (t > 4) return;
    var c = generate(t);
    for (var i in c) {
        if (is_balanced(coefficients(c[i], eq))) {
            return c[i];
        }
    }
}
